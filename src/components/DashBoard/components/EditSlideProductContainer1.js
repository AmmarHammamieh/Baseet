...
import { removeavailableProduct, selectAvailableProducts } from '../../../GlobalData/AvailableProductsSlice';
...
function EditSlideProductContainer1({id,image , name,description,price,category,style}) {
  let Id = id;
  ...
  function Check(e, id) {
      ...
      for (let i = 0; i < AvailableProducts.length; i++) {
        if (AvailableProducts[i].id == id) {
          dispatch(addinsideSlideProduct(AvailableProducts[i]));
        }
      }
      dispatch(removeavailableProduct({id}));
...
export default EditSlideProductContainer1