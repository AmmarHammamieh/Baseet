...
function ProductShower2({Product,ApprovedProduct,id,image , name,description,price,category}) {
  let Id = id;
    ...
    if (ApprovedProduct.length > 0) {
      for (let i = 0; i < ApprovedProduct.length; i++) {
        if (ApprovedProduct[i].id === id) {
          dispatch(addToBasket(ApprovedProduct[i]));
        }
      }
      dispatch(removeFromApproved({ id }))
