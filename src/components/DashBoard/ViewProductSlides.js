...
import SlideTestProducts from './Modals/SlideTestProducts';
import ProductSlidesTable from './Modals/ProductSlidesTable';
import { useDispatch, useSelector } from 'react-redux';
import { addProductSlide, selectProductSlide , } from '../../GlobalData/SlideProductSlice';
import { addImageSlide, selectImageSlide } from '../../GlobalData/ImageSlideSlice';
import ImageSlidesTable from './Modals/ImageSlidesTable';
import { RiSlideshowLine } from 'react-icons/ri';
import { Link } from 'react-router-dom';
import Cookies from 'universal-cookie';

function ViewProductSlides() {
   const [slides, setSlides] = useState([]);
   const [ImageSlides, setImageSlides] = useState([]);
   
    const dispatch = useDispatch();
    const selectProductslide = useSelector(selectProductSlide);
    const selectImageslide = useSelector(selectImageSlide);
  ...
  return (
    <div className='w-full h-full'>
        ...
        <div className='flex bg-[#F5F7F9]'>  
              ...
              <div className='mt-52 w-3/4 h-full flex flex-col  justify-center items-center'>
                  
              {console.log(selectProductslide)}
              <p className='w-3/4 text-right mr-3'>شرائح منتجات</p>
                 <ProductSlidesTable itemsPerPage={4} Data={selectProductslide[0]} />
                 
              {/* SlideImages */}
              <div className='mt-52 w-full flex flex-col  justify-center items-center'>
                 <p className='w-3/4 text-right mr-3'>شرائح صور</p>
              {console.log(selectImageslide,"hi")}

                 <ImageSlidesTable itemsPerPage={4} Data={selectImageslide[0]} />
                 </div>
                </div>
...
export default ViewProductSlides